...
def add_dataset_parser(parser: FlexibleArgumentParser):
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "custom", "prefix_repetition", "spec_bench", "tool_calling"
        ],
    ...
    tool_calling_group = parser.add_argument_group("tool calling dataset options")
...
def get_samples(args, tokenizer) -> list[SampleRequest]:
    ...
    elif args.dataset_name == "tool_calling":
...
# Tooling Dataset
# -----------------------------------------------------------------------------
class ToolCallingDataset(BenchmarkDataset):
    """Dataset for benchmarking tool calling capabilities with configurable input/output lengths.
    
    This dataset generates synthetic tool calling prompts with long context to test
    model performance on tool use tasks with extended input sequences.
    """
    
    def __init__(self, dataset_path: Optional[str] = None, random_seed: int = 0):
        """Initialize the ToolCallingDataset.
        
        Args:
            dataset_path: Optional path (unused for synthetic generation)
            random_seed: Seed for reproducible generation
        """
        super().__init__(dataset_path=dataset_path, random_seed=random_seed)
        
    def load_data(self):
        """Load data - no-op for synthetic dataset."""
        pass
...
The path forward requires collaboration among researchers, policymakers, industry leaders, and civil society organizations to ensure that AI development proceeds in ways that maximize benefits while minimizing risks. This includes investment in AI safety research, development of robust testing and evaluation methodologies, creation of inclusive stakeholder engagement processes, and establishment of international cooperation frameworks for addressing global challenges and opportunities presented by artificial intelligence technologies."""
    ...
    def create_tool_calling_prompts(self, input_tokens: int = 122880, output_tokens: int = 8192) -> list[str]:
        ...
        Args:
            input_tokens: Target input token length
            output_tokens: Expected output token length
    ...
    def sample(
        ...
        **kwargs
        ...
        Args:
            ...
            input_tokens: Target input token length (default: 122880)
            output_tokens: Expected output token length (default: 8192)
            tool_subset: Optional list of specific tool names to include
